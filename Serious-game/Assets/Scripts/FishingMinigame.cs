...
public class FishingMinigame : MonoBehaviour
{
    ...
    private void Update() {
	    if (!reelingFish) { //If we arent currently in the reeling stage
		    if (action.triggered && !lineCast) { //This is if we are doing nothing and are ready to cast a line
			    CastLine();
		    }else if (action.triggered && lineCast && !nibble) { //This is if the line has cast and we reel in before we get a nibble
		    ...
		    }else if (action.triggered && lineCast && nibble) { //This is if we reel in while there is a nibble
			    StopAllCoroutines(); //Stops the LineBreak timer
			    StartReeling();
		    }
	    } else { //This is when we are in the stage where we are fighitng for the fish
		    if (action.triggered) { //If we press space
